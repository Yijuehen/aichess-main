...
if CONFIG['use_frame'] == 'paddle':
    ...
    print(f'UI使用设备: {policy_value_net.device}')
else:
    print('暂不支持您选择的框架')
...
# Human plays as Red (player1, moves first)
player1 = Human()

# AI plays as Black (player2)
player2 = MCTSPlayer(policy_value_net.policy_value_fn,
                     c_puct=5,
                     n_playout=200,  # Optimized for speed (~1.7s per move)
                     is_selfplay=0)

# For AI vs AI, uncomment these lines instead:
    ...
    if player_in_turn.agent == 'AI':
        ...
        if len(move_action) == 4:
            ...
            if move != -1 and move in board.availables:
                # Only allow the move if it's legal and belongs to current player
                board.do_move(move)  # 棋盘做出改变
                swicth_player = True
                move_action = ''
                draw_fire = False
